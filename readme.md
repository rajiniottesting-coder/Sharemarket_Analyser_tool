...
| Version | Key change |
| **v15.0** | **A- technical rigor + Performance-sheet reset + tooltip/glossary updates.** **5 enhancements over v14.6**: 5-tier sector classification, ATR-percentile regime detection (using 252-day baseline, full 400-day retention window), volume-confirmed support floor, trailing stops with no-lookahead discipline (G17 regression-tested), earnings-near SL widening (logic ready, data source pending). **Performance sheet wipe**: `wipe_v15_prep.py` with mandatory backup + `--restore` option. **Schema migrations**: idempotent ALTER TABLE adds original_stop_loss, atr_at_rec, regime_at_rec, next_earnings_date, trailing_sl_pct, trailing_sl_price, peak_price_seen. **Tooltips updated**: SL/T1/T2/T3/R:R/Time-Horizon tooltips in `reporting/tooltip_formatter.py` reflect v15.0 multi-factor formula + trailing-SL behavior. **Glossary entries added**: 3 new v15.0 entries in Performance sheet glossary explaining SL/T methodology, trailing-stop logic, and sector-tier/regime/volume-confirmation audit columns. **Files**: master_funnel.py, track_outcomes.py, database/data_bridge.py, reporting/tooltip_formatter.py, reporting/excel_generator.py, test_v13_v14_consolidated.py, wipe_v15_prep.py. **69/69 tests** pass across 5/5 consecutive runs. Honest grade: A- (smart heuristics, 4 fully wired; true A still needs walk-forward backtest calibration). |
...
**Visual pipeline reference:** [pipeline_reference_v15_0.html](pipeline_reference_v15_0.html)
