...
| Version | Key change |
| **v14.5** | **CLOSED POSITIONS section added to Performance sheet — chronological track-record log.** User observation: "in the performance sheet good to have closed position section which should hold information such as expired stocks with expired date and stock price at expired date and other relevant info — gives more insight on the track record." Pre-v14.5 the sheet showed only aggregated counts (T1 HIT: 21, SL HIT: 10, etc.) in headline tiles — no way to see individual closures. Fix: added a new section between DIAGNOSTIC BREAKDOWNS and OPEN POSITIONS displaying every closed pick (SL_HIT/T1_HIT/T2_HIT/T3_HIT/EXPIRED) as a 12-column table sorted by outcome_date DESC. Columns: Symbol · Rec Date · Time Horizon · Outcome · Outcome Date · Days to Outcome · Entry CMP · Outcome Price · P&L % · Max Runup % · Max Drawdown % · Score. Outcome column is color-coded (green T1/T2/T3, red SL, amber EXPIRED) for at-a-glance scanning. Realised P&L computed live from entry vs outcome price. Summary footer counts each outcome bucket. Empty-state: "No closed positions yet — track record will populate as picks resolve." Single-file code change: `reporting/excel_generator.py`. **New regression test**: `test_g14_closed_positions_section_renders_correctly` (verifies section order, headers, sort, P&L formula, summary). **66/66 tests pass** across 5 consecutive runs. **Deploy**: `reporting/excel_generator.py` + `test_v13_v14_consolidated.py` (optional). |
