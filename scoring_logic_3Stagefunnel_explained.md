...
### A. Resist 2 / Support 2 — now prior 52-week levels (v12.4 / v12.6)
...
**v12.6 follow-up:** v12.5 production audit found 86 of 99 stocks STILL showing `R1 == R2` even though the v12.4 patch was demonstrably running (78 % of stocks had 52W High > R2 by ≥5 %, the post-patch signature). Root cause: for stocks bouncing in a narrow range for 9+ months (52W high happened earlier than the 252-day prior window), the rolling-252 max over `prior_h` happens to equal the recent 20-day max. Mathematically correct, visually unhelpful. v12.6 adds a tolerance check: if `|R1-R2|/R1 < 0.5 %`, R2 falls back to NaN → renders `"—"`. The user reads "no prior ceiling distinct from the recent swing high — treat R1 as both T1 and T2/T3", which is the honest signal the data is conveying.
...
**Document version:** reflects code as of v12.5 (quality-of-life fixes). Scoring logic (Parts 2-3) unchanged since v10.9 except for v10.16 PE-scoring-neutrality-for-clamped-values addition. Funnel (Part 1) last changed in v10.13. v10.14 added GROWTH field clamps + tooltips. v10.15 extended clamp discipline to PROFITABILITY/FIN-HEALTH/VALUATION/SHAREHOLDING + fixed ROE/ROA numeric storage + honest "—" display for Pledge%/DII%/QoQ. v10.16 replaced v10.15's numeric clamp for valuation ratios with honest "—" display (raw ≥ 500 / PEG ≥ 50) and added scoring neutrality for clamped PE (pe_num ≥ 500 = neutral, not penalized). v11.0.1 corrected EE threshold from ≥70 to ≥50 in daily report and Section D rotation_stage matching. v11.0.2 added 4 features: allowlist auto-add (A1), allowlist auto-prune (A2), chronic-AVOID demotion (B), turnaround flag (C). v12.0 added BSE bhavcopy 3-tier Cloudflare-resilient cascade (bse → cloudscraper → curl_cffi) in `master_funnel._bse_bhav` and removed the silent NEUTRAL filter in `ExcelGeneratorV6.__init__` so Stage 3 is now the single quality gate. **v12.1** is a reconciler hotfix preventing the empty-ISIN Cartesian-merge false positive that surfaced once v12.0 restored real BSE data: empty-ISIN rows now default to NSE_ONLY/BSE_ONLY instead of being incorrectly tagged DUAL_LISTED via symbol-match. **v12.4** ships four production-blocker fixes: (1) header-demotion threshold raised from ≥1 row to ≥30 % of rows so sparse columns (Pro QoQ Δ, FII QoQ Δ) correctly stay red; (2) Resist 2 / Support 2 now compute over bars BEFORE the most recent 20, fixing the 87.9 % R1==R2 collapse on fresh breakouts that the v10.9 attempt missed; (3) profitability values (NPM / ROA / Gross / EBITDA margins) clamped to ±100 % with display + numeric guards, eliminating six rows of impossible >100 % margins; (4) all user-facing strings switched from "Anthropic API credits" / "Claude AI" to "Gemini API credits" / "Gemini AI" to match the actual provider (DB column names `last_claude_score`, `claude_analysed` deliberately preserved to avoid a schema migration). Locked behind 41 new regression tests (Group 53 in `test_v11.0.2_full_withdummies.py`). Zero DB schema change, zero scoring behaviour change beyond the bounded inputs to roe_num/gm_num/nm_num. **v12.5** ships six quality-of-life fixes: (1) MoS Label gets a trailing `*` when the FV engine clips CFV at 3× CMP (new `cfv_capped` flag in fair_value_engine.py output, preserved by master_funnel through the label override); (2) Gold sheet uses the same coverage-based dynamic header demotion as Full Dashboard (`_GOLD_COV_MIN = 0.30`); (3) Gold column renamed `F-Score /9` → `Piotroski F /9` to match Full Dashboard, three orphaned doc entries cleaned up; (4) Early Mover dedup uses prefix-match (`_has_prefix(sigs, "EARLY MOVER")`) instead of exact-match — pre-fix the badge and the label were both appended for 8 stocks because they were different strings; (5) Altman Z clamped to `[0, 10]` in `forensics_engine.py` — Z>7 already signals exceptional safety, and ALIVUS 14.69 / GOPAL 17.27 / CPEDU 26.70 in production were unit-mismatch artefacts; (6) CCC Days renders `—` for finance-sector stocks (sector contains `financial`/`finance`/`bank`/`nbfc`/`insurance`/`housing finance`) — the metric is meaningless for them (TATACAP showed 7,739 days). Locked behind 30 new regression tests (Group 54). Issue #3 (0-vs-missing ambiguity) was investigated but deferred — root cause is SQL-layer `COALESCE(.., 0)` that destroys the distinction before data reaches Python. Issues #2, #4, #11, #14 remain as judgment calls awaiting product input. Zero DB schema change, zero scoring behaviour change. **v12.6** ships five final-round judgment-call fixes: (1) Resist 2 / Support 2 cells render `"—"` when the prior 252-day window's max is within 0.5 % of the recent 20-day max (new `_R2_TOLERANCE = 0.005` constant in `backfill_history.py`); v12.5 production showed 86 / 99 stocks with R1 == R2 due to legitimate post-patch behavior on stocks with narrow 9+ month trading ranges — the duplicate display gave traders no signal and is now an honest "no distinct prior ceiling" `"—"`; (2) `mos_label` is no longer set inside `fair_value_engine.get_composite_fair_value` — the engine's 7-bucket scheme was unreachable code because master_funnel always overwrote it with its own 6-bucket scheme; engine output now emits `cfv`, `mos_pct`, `cfv_capped`, `cfv_thin_models`, `n_models`, `score_adjustment`, `upside` only; (3) fair-value thin-model quality guard with `MIN_MODELS = 3` — when fewer than 3 of M1–M7 fired, `score_adjustment` is forced to 0 regardless of MoS magnitude, and a new `cfv_thin_models` flag is True; master_funnel appends `†` to MoS Label so users see the thin-evidence signal; this prevents thin-FV stocks from being promoted to BUY purely on a 1-or-2-model MoS bonus; (4) NPM column headers renamed `Q1 % / Q2 % / Q3 %` → `Q (latest) % / Q-1 % / Q-2 %` to fix the inverse-chronological confusion (Q1 was actually the LATEST quarter despite L→R reading order suggesting otherwise); DB column keys (`npm_q1`, `npm_q2`, `npm_q3`) deliberately preserved — display-only label change, zero scoring impact; (5) all four "no AI analysis" placeholder strings unified to `[AI <verb> — <reason>]` format (`[AI not yet generated — Analysis pending]`, `[AI skipped — verdict AVOID, ...]`, `[AI skipped — Gemini API quota exhausted (batch N), ...]`, `[AI unavailable — Gemini returned empty response ...]`) — distinct meanings preserved, format now consistent for grep / filtering. Locked behind 30 new regression tests (Group 55) for **475/475 total**. Issue #3 (0-vs-missing ambiguity) remains the only unresolved audit issue — deferred to a dedicated future release because the architectural rewrite (SQL `COALESCE` removal across multiple queries plus downstream `None`-handling refactor) is too risky to bundle with display-layer changes. Sanity check confirmed end-to-end scoring/verdict logic intact: full-FV stocks unchanged (composite=70.75, BUY); thin-FV stocks correctly NOT promoted (composite=48.26, NEUTRAL — was 60.26, BUY pre-fix); AVOID floor still working; cap-tier thresholds (LARGE/MID/SMALL/MICRO) unchanged.
